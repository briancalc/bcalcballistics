...
def calcFR(temperature, barometer, relative_humidity):
    """
    ...
    frh = 0.995*(barometer/(barometer-(0.3783)*(relative_humidity)*VPw))
    return frh


def calcFP(barometer):
    p_std = 29.92  # in-hg; standard pressure at sea level
    fp = (barometer - p_std) / (p_std)
    return fp
...
#strange, this appears to be using drag_coefficient for what other files call drag_function (G1 or G7).  the notes say it is correcting the standard drag coeffcient
#but then refering to it as G1 or G7 rather than a numerical value
