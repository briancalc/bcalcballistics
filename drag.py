#drag.py
#supplies the estimated decleration (drag force per unit mass) at a specific velocity given an original BC (drag_coefficient)
#another way to state, at this speed with this BC, drag force is (result of this file) so bullet slows by this ft/s^2 (not just ft/s)
...
def G7(vp):
    """
    Smoothed G7 drag model. Tries to preserves the original return
    contract but smoothly blends coefficients near band boundaries
    """
    ...
    # original G7 coefficient table
    # format:  (minimum velocity for band, acceleration, mass)
    bands = [
        (4200, 1.29081656775919e-09, 3.24121295355962),
        ...
        (1110, 5.71086414289273e-12, 4.32128262648890),
        (960,  3.02865108244904e-17, 5.99074203776707),
        (670,  7.52285155782535e-06, 2.17380198510750),
        (540,  1.31766281225189e-05, 2.08774690257991),
        (0,    1.34504843776525e-05, 2.08702306738884),
    ]

    # transition widths centered on actual boundaries
    #example: 3000 boundary with width 100: smooth region = 2950 -> 3050
    transition_widths = {
        4200: 100,
        3000: 100,
        1470: 50,
    ...
    for i in range(len(bands) - 1):
        ...
        boundary, upper_a, upper_m = bands[i]
        _, lower_a, lower_m = bands[i + 1]

        width = transition_widths.get(boundary, 0)

        half_width = width / 2.0

        lower_edge = boundary - half_width
        upper_edge = boundary + half_width

        # Smooth transition zone
        if lower_edge <= vp <= upper_edge:

            # Blend factor
            t = (vp - lower_edge) / width

            # Compute actual drag from each regime
            drag_lower = drag(lower_a, lower_m, vp)
            drag_upper = drag(upper_a, upper_m, vp)

            # Blend drag directly
            blended_drag = lerp(drag_lower, drag_upper, t)

            # Blend exponent only for equivalent representation
            blended_m = lerp(lower_m, upper_m, t)

            # Reconstruct equivalent acceleration coefficient
            blended_a = blended_drag / math.pow(vp, blended_m)

            return blended_a, blended_m

        # Pure upper band region
        if vp > lower_edge:
            return upper_a, upper_m
...
def retard(drag_function, drag_coefficient, vp):
    ...
    func = drag_functions.get(drag_function)
    if func is None:
        raise ValueError("Unsupported drag function")

    acceleration, mass = func(vp)
